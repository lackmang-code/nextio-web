...
**⚠️ 반드시 지킬 것**
- **`digest_to_items.py`(디스플레이 실발행 전용)와 `run_daily.py`는 건드리지 말 것.** 쇼케이스는 전부 별도 파일로 분리돼 있다.
...
⚠️ **실운영 디스플레이 데일리에도 같은 문제가 있다** — 8/23분에 8/16·8/19자 기사가 2건 섞여 나갔다. 발행 중인 코드라 아직 적용하지 않았고, **대표 지시를 받고 진행할 사항**이다.
